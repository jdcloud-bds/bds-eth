...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, []interface{}, []*types.Log, error) {
	...
	for ; block != nil && err == nil; block, err = it.next() {
		...
		receipts, logs, traceReses, usedGas, err := bc.processor.ProcessWithTrace(block, state, bc.vmConfig)
		t1 := time.Now()
		if err != nil {
...
func (bc *BlockChain) WriteDataToKafka(blk *types.Block, rcps types.Receipts, signer types.Signer, traceResult []*types.TraceResult) error {
	...
	b.Block.Timestamp = blk.Time().Int64()
	...
	if len(blk.Uncles()) > 0 {
		for _, uncle := range blk.Uncles() {
			...
			item.Timestamp = uncle.Time.Int64()
